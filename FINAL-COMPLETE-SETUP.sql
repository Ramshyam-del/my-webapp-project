-- =====================================================
-- QUANTEX TRADING PLATFORM - FINAL COMPLETE SETUP
-- =====================================================
...
-- Drop existing policies to avoid conflicts (only if they exist)
DO $$
BEGIN
    -- Users policies
    ...
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'configuration' AND policyname = 'Anyone can view configuration') THEN
        DROP POLICY "Anyone can view configuration" ON public.configuration;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'configuration' AND policyname = 'Only admins can modify configuration') THEN
        DROP POLICY "Only admins can modify configuration" ON public.configuration;
    END IF;

    -- Crypto pairs policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'crypto_pairs' AND policyname = 'Anyone can view crypto pairs') THEN
        DROP POLICY "Anyone can view crypto pairs" ON public.crypto_pairs;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'crypto_pairs' AND policyname = 'Only admins can modify crypto pairs') THEN
        DROP POLICY "Only admins can modify crypto pairs" ON public.crypto_pairs;
    END IF;
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'orders' AND policyname = 'Users can create own orders') THEN
        DROP POLICY "Users can create own orders" ON public.orders;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'orders' AND policyname = 'Users can update own orders') THEN
        DROP POLICY "Users can update own orders" ON public.orders;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'orders' AND policyname = 'Admins can view all orders') THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'transactions' AND policyname = 'Users can create own transactions') THEN
        DROP POLICY "Users can create own transactions" ON public.transactions;
    END IF;
    ...
    -- Trades policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'trades' AND policyname = 'Anyone can view trades') THEN
        DROP POLICY "Anyone can view trades" ON public.trades;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'trades' AND policyname = 'Only admins can create trades') THEN
        DROP POLICY "Only admins can create trades" ON public.trades;
    END IF;

    -- Wallets policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wallets' AND policyname = 'Users can view own wallets') THEN
        DROP POLICY "Users can view own wallets" ON public.wallets;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wallets' AND policyname = 'Users can create own wallets') THEN
        DROP POLICY "Users can create own wallets" ON public.wallets;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wallets' AND policyname = 'Users can update own wallets') THEN
        DROP POLICY "Users can update own wallets" ON public.wallets;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'wallets' AND policyname = 'Admins can view all wallets') THEN
        DROP POLICY "Admins can view all wallets" ON public.wallets;
    END IF;

    -- Deposits policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND policyname = 'Users can view own deposits') THEN
        DROP POLICY "Users can view own deposits" ON public.deposits;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND policyname = 'Users can create own deposits') THEN
        DROP POLICY "Users can create own deposits" ON public.deposits;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND policyname = 'Admins can view all deposits') THEN
        DROP POLICY "Admins can view all deposits" ON public.deposits;
    END IF;

    -- Withdrawals policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawals' AND policyname = 'Users can view own withdrawals') THEN
        DROP POLICY "Users can view own withdrawals" ON public.withdrawals;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawals' AND policyname = 'Users can create own withdrawals') THEN
        DROP POLICY "Users can create own withdrawals" ON public.withdrawals;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawals' AND policyname = 'Admins can view all withdrawals') THEN
        DROP POLICY "Admins can view all withdrawals" ON public.withdrawals;
    END IF;

    -- Mining pools policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mining_pools' AND policyname = 'Anyone can view mining pools') THEN
        DROP POLICY "Anyone can view mining pools" ON public.mining_pools;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mining_pools' AND policyname = 'Only admins can modify mining pools') THEN
        DROP POLICY "Only admins can modify mining pools" ON public.mining_pools;
    END IF;

    -- Mining payouts policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mining_payouts' AND policyname = 'Users can view own mining payouts') THEN
        DROP POLICY "Users can view own mining payouts" ON public.mining_payouts;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'mining_payouts' AND policyname = 'Admins can view all mining payouts') THEN
        DROP POLICY "Admins can view all mining payouts" ON public.mining_payouts;
    END IF;

    -- Operation logs policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'operation_logs' AND policyname = 'Only admins can view operation logs') THEN
        DROP POLICY "Only admins can view operation logs" ON public.operation_logs;
    END IF;

    -- System notifications policies
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'system_notifications' AND policyname = 'Anyone can view system notifications') THEN
        DROP POLICY "Anyone can view system notifications" ON public.system_notifications;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'system_notifications' AND policyname = 'Only admins can modify system notifications') THEN
        DROP POLICY "Only admins can modify system notifications" ON public.system_notifications;
    END IF;

EXCEPTION
    WHEN OTHERS THEN
        -- Continue if any policy doesn't exist
        NULL;
END $$;

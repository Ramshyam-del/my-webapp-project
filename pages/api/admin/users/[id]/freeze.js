...
export default async function handler(req, res) {
  try {
    ...
    // Update user account status to Frozen
    const { data: updatedUser, error: updateErr } = await server
      .from('users')
      .update({ 
    ...
    if (updateErr) {
      return res.status(500).json({ ok: false, code: 'UPDATE_ERROR', message: 'Failed to freeze account' })
    }

    // Log the action
    console.log(`Account frozen for user ${targetUser.email} (${targetUser.id}) by admin ${userId}`)
    ...
    console.error('Freeze account error:', e)

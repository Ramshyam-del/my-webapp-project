...
export default async function handler(req, res) {
  try {
    ...
    if (!['Normal', 'Frozen', 'Disabled'].includes(status)) {
      return res.status(400).json({ ok: false, code: 'INVALID_STATUS', message: 'Status must be Normal, Frozen, or Disabled' })
    }
    ...
    if (updateErr) {
      return res.status(500).json({ ok: false, code: 'UPDATE_ERROR', message: 'Failed to update account status' })
    }

    return res.status(200).json({ ok: true, data: user })
  } catch (e) {
    console.error('Account status update error:', e)
